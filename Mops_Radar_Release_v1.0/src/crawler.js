...
const TYPE_KEYWORDS = {
  clarify:   ['澄清', '媒體', '報導', '報載', '雜誌', '內容', '新聞'],
  meeting:   ['股東會', '董事會', '股東常會', '股東臨時會', '召開', '決議', '議案', '法說會', '業績說明會'],
  financial: ['財務報告', '季報', '年報', '半年報', '財報', '損益', '獲利', '盈餘', '自結', '營收'],
  dividend:  ['股利', '除權', '除息', '盈餘分配', '盈餘分派', '現金', '股票', '配息', '配股', '配發'],
  capital_change: ['可轉債','增資', '私募', '可轉換公司債', '庫藏股', '買回股份', '減資', '發行新股', '公司債', 'CB', '現增', '註銷', '轉換'],
  merger:    ['合併', '收購', '分割', '購併', '併購'],
  board:     ['辭職', '卸任', '委任', '派任', '解任', '辭任', '任期屆滿'],
  asset_disposal: ['處分', '取得', '資產', '不動產', '機器', '設備', '廠房', '工程', '土地'],
};
...
function classifyType(title) {
  ...
  for (const [type, keywords] of Object.entries(TYPE_KEYWORDS)) {
    if (keywords.some(k => title.includes(k))) {
      // 排除邏輯
      if (type === 'merger' && (title.includes('合併財務報告') || title.includes('合併報表') || title.includes('合併營收'))) {
        continue; // 這是財報，不是併購，跳過
      }
  ...
  // 如果都沒對到，歸類為其他重大訊息
  if (matchedTypes.length === 0) matchedTypes.push('major');
